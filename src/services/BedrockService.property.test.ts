 ...
 *     - `transcript.length <= 100_000` → SDK `send`가 호출되고 요청이 정상 개시된다.
 *     - `transcript.length >  100_000` → SDK `send`가 호출되지 않으며
 ...
 *   `"a".repeat(n)`으로 구성한다(랜덤 코드포인트 생성은 길이 10만 기준으로 과도하게 느리다).
 * - 실행 횟수는 `numRuns: 50` — 길이 범위 [0, 100_000] 및 (100_000, 120_000]을
...
describe("BedrockService.analyze — Property 10: 본문 길이 경계 검증 규칙", () => {
	...
	// -------------------------------------------------------------------------
	// (b) over-limit: length ∈ (100_000, 120_000] → send NOT called, throws
	// -------------------------------------------------------------------------
	test("transcript.length > 100_000 → send 미호출 + TRANSCRIPT_TOO_LONG throw", async () => {
		const service = new BedrockService(realClientFactory);

		await fc.assert(
			fc.asyncProperty(
				// 경계(100_000)를 살짝 넘기는 값부터 여유를 둔 상한까지 고르게 샘플.
				// 상한은 실행 시간을 적정하게 유지하면서도 경계를 충분히 벗어나도록 120_000 사용.
				fc.integer({
	...
	// -------------------------------------------------------------------------
	// (c) 경계값: length === 100_000 (포함 경계)
	//     Property 10의 "<= 100_000 이면 호출"을 명시적 예시로 확인한다.
	// -------------------------------------------------------------------------
	test("경계: transcript.length === 100_000 → send가 호출된다", async () => {
		resetMock();
		const service = new BedrockService(realClientFactory);
	...
	// -------------------------------------------------------------------------
	// (d) 경계값: length === 100_001 (미포함 경계)
	//     "length > 100_000이면 미호출 + throw"를 명시적 예시로 확인한다.
	// -------------------------------------------------------------------------
	test("경계: transcript.length === 100_001 → send 미호출 + throw", async () => {
		resetMock();
		const service = new BedrockService(realClientFactory);
