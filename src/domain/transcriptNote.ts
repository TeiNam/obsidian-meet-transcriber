 ...
 *
 * ## 식별 전략
 * 원본 섹션은 콜아웃 **타입**(`quote`)으로 식별한다. 제목 문자열은 로케일에 따라
 * 다르므로(원본/Original) 파싱 기준으로 쓰지 않는다. 분석 섹션은 박스(콜아웃) 없이
 * 평문이며, "본문 맨 위 ~ 첫 콜아웃 직전" 영역으로 본다.
 *
 * `tip` 콜아웃(과거 AI 교정본 기능이 만들던 섹션)은 더 이상 생성되지 않지만, 이미
 * 저장된 노트에 남아 있을 수 있으므로 `CALLOUT_REFINED` 식별자와 관련 읽기 경로는
 * 레거시 호환을 위해 유지한다(Requirement: 기존 노트 데이터 보존).
 *
...
/** (레거시) AI 교정본 콜아웃 타입. 신규 생성되지 않으며 기존 노트 읽기용으로만 참조된다. */
...
export interface CalloutTitles {
	analysis: string;
	original: string;
	/** 레거시 `tip` 콜아웃을 이어쓰기 시 재직렬화할 때만 사용된다. */
	refined: string;
}
