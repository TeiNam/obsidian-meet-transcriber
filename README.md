...
- 전사 종료 시 `Transcribe-YYYYMMDD-HHmmss.md` 형식으로 마크다운 노트 자동 저장 (프론트매터 포함)
- 저장된 노트의 직접 편집(후보정) 지원
- AWS Bedrock 을 통한 요약·키워드·액션 아이템 분석 결과 노트에 부착
- 영어 / 한국어 UI (설정에서 전환)
...
| Analysis | Bedrock model ID | 분석에 사용할 Bedrock 모델 식별자 (예: `anthropic.claude-3-sonnet-20240229-v1:0`) |

### 사용 방법

1. 명령 팔레트 또는 리본 아이콘의 `Open transcribe view` 명령으로 사이드바를 엽니다.
2. **Start streaming** 버튼을 눌러 마이크 권한을 허용합니다.
3. 발화를 시작하면 잠정(희미한 글씨) → 확정(일반 글씨) 순으로 텍스트가 누적됩니다.
4. **Stop streaming** 을 누르면 누적된 내용이 `Transcribe-YYYYMMDD-HHmmss.md` 로 저장됩니다.
5. **Edit** 버튼으로 오탈자를 교정하거나, **Analyze** 버튼으로 Bedrock 분석 결과를 노트 끝에 `## 분석 결과` (또는 `## Analysis result`) 섹션으로 추가합니다.
...
- Auto-save the transcript as `Transcribe-YYYYMMDD-HHmmss.md` with YAML front matter
- Post-edit the saved transcript directly from the sidebar
- Append AWS Bedrock analysis (summary, keywords, action items) to the note
...
| Analysis | Bedrock model ID | Bedrock foundation model identifier (e.g. `anthropic.claude-3-sonnet-20240229-v1:0`) |
...
3. As you speak, partial results appear in a muted style and are replaced by final results as the service confirms them.
4. Click **Stop streaming** to save the transcript as `Transcribe-YYYYMMDD-HHmmss.md`.
5. Use **Edit** to correct errors, or **Analyze** to append a Bedrock-generated summary under an `## Analysis result` (or `## 분석 결과`) section.
